...
(function Engine(){
  ...
  function chrome(step){
    ...
    var h = '<div class="topbar">'
      ...
      + '<div class="brand-tx"><b>Cola de Arroz <span>Roxo</span></b><small>Teste de Pele da Dra. Yuna</small></div></div>'
  ...
  function tCard(t){
    ...
    if (t.before && t.after) {
      media = '<div class="ba-pair">'
        ...
        + '<div class="half"><span class="ba-tag after">Depois</span><img src="' + t.after + '" alt=""></div></div>';
    } else {
  ...
  function baCard(b, withMeta){
    return '<div class="ba-card"><div class="ba-pair">'
      ...
      + '<div class="half"><span class="ba-tag after">Depois</span><img src="' + b.after + '" alt=""></div></div>'
  ...
  var renderers = {
    ...
    landing: function(s){
      ...
      return '<div class="screen">'
        + '<div class="hero-img"><img src="' + s.heroImg + '" alt="Cola de Arroz Roxo Coreano" fetchpriority="high"></div>'
    ...
    name: function(s){
      return '<div class="screen"><h1 class="q-title">' + s.title + '</h1>'
        + '<p class="q-sub">' + s.sub + '</p>'
        + '<input class="name-input" id="name-input" type="text" maxlength="24" autocomplete="given-name" placeholder="Seu primeiro nome" value="' + esc(QuizState.get('name') || '') + '">'
    ...
    break: function(s){
      ...
      return '<div class="screen inter">'
        + '<span class="kicker-tag">Você não está sozinha</span>'
        + '<h2>Está tudo bem, ' + esc(QuizState.get('name') || 'amiga') + '! Milhares das mais de 37 mil mulheres que rejuvenesceram com a Cola de Arroz Roxo Coreano passaram exatamente por isso.</h2>'
        + '<p class="body" style="text-align:center">Veja o que elas dizem sobre os métodos que <b>você</b> tentou:</p>'
    ...
    emotional: function(s){
      ...
      if (s.img2) {
        h += '<div class="ba-card" style="margin-bottom:12px"><div class="ba-pair">'
          ...
          + '<div class="half"><span class="ba-tag after">Depois</span><img src="' + s.img2 + '" alt=""></div>'
          + '</div></div>';
      } else {
    ...
    diagnosis: function(s){
      ...
      var chart = '<svg viewBox="0 0 320 130" xmlns="http://www.w3.org/2000/svg">'
        ...
        + '<text x="312" y="15" font-size="9.5" font-weight="800" fill="#2E9E5B" text-anchor="end" font-family="Inter,sans-serif">SUA META</text>'
        ...
        + '<text x="34" y="86" font-size="9" font-weight="800" fill="#D64545" text-anchor="start" font-family="Inter,sans-serif">VOCÊ HOJE</text>'
        + '<circle cx="306" cy="25" r="6" fill="#2E9E5B"/>'
        + '<text x="34" y="120" font-size="9" font-weight="700" fill="#6B7080" font-family="Inter,sans-serif">Hoje</text>'
        ...
        + '<text x="306" y="120" font-size="9" font-weight="700" fill="#6B7080" text-anchor="end" font-family="Inter,sans-serif">Dia 21</text>'
        + '</svg>';
      return '<div class="screen">'
        + '<div class="diag-head"><img src="assets/t25-dryuna-diagnosis.jpg" alt="Dr. Yuna Mun">'
        + '<div><div class="dh-t">Avaliação da Dra. Yuna para ' + esc(name) + (age ? ', ' + age : '') + '</div>'
        + '<div class="dh-s">Com base nas SUAS respostas · comparado ao método clínico dela</div></div></div>'
        + '<div class="diag-box-red">'
        + '<div class="status-line">Status do Escudo da Pele</div>'
        + '<div class="status-value">' + d.status.label + '</div>'
        + '<div class="status-gauge">' + gauge + '</div>'
        + '<div class="gauge-labels"><span>Desligado</span><span>Enfraquecido</span><span>Ligado</span></div>'
        + '<div class="diag-alert-title">⚠️ O que encontrei nas SUAS respostas:</div>'
        + findingsHtml + '</div>'
        + '<div class="diag-box-green">💚 <b>A boa notícia: o escudo pode ser RELIGADO — comprovado em mulheres ' + (age || 'da sua idade') + '.</b> A Cola de Arroz Roxo Coreano — na proporção certa pra SUA pele — é o interruptor. ✨</div>'
        + '<div class="proj-chart"><div class="ng-title">Sua progressão prevista — próximos 21 dias</div>' + chart + '</div>'
    ...
    futurepacing: function(s){
      ...
      return '<div class="screen inter">'
        + '<span class="kicker-tag">Sua projeção</span>'
        + '<h2>' + fp.title + '</h2>'
        + '<div class="fp-hero"><img src="assets/sales-macro-bowl.jpg" alt="Cola de Arroz Roxo Coreano"></div>'
  ...
  function showQuizExitPopup(){
    ...
    div.innerHTML = '<div class="popup">'
      ...
      + '<h3>Espera' + (name ? ', ' + name : '') + ' — o seu plano de 21 dias da Cola de Arroz Roxo Coreano está pronto!</h3>'
      + '<p>Suas respostas já foram analisadas e o seu ritual personalizado está <b>gerado e te esperando</b>. Não perca — é só um toque.</p>'
      + '<button class="btn" id="exit-cta">Quero meu acesso →</button>'
      + '<button class="later" id="exit-later">Não, quero continuar respondendo</button>'
      + '</div>';
