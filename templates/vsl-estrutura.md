...
1. **Hook (5s)** — pattern interrupt + promessa específica + loop aberto. Se a VSL roda pós-quiz, **PERSONALIZAR** (nome/resultado/desafio).
...
6. **Objeção-chave quebrada** — a maior resistência do nicho ("já tentei e não funcionou → faltava [o seu mecanismo]").
...
## Regras (checklist)
- [ ] **Personalização** quando pós-quiz (nome/resultado/desafio na tela; áudio genérico se for arquivo fixo).
...
- [ ] **Amarração ao mecanismo**: ligar a dor à causa que só o seu mecanismo resolve.
...
- Preencha a partir do `05-diferenciacao.md` + `08-copy.md` da sua oferta.
